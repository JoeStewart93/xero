...
BEACON_TOKEN_HASH_PREFIX = "sha256:"
...
def create_c2_access_token(settings: Settings, *, now: datetime | None = None) -> tuple[str, datetime]:
    issued_at = now or datetime.now(UTC)
...
def decode_c2_access_token(token: str, settings: Settings) -> dict[str, Any]:
    try:
        claims = jwt.decode(
            token,
            settings.jwt_secret_key,
...
def hash_beacon_token(token: str) -> str:
    ...
    return f"{BEACON_TOKEN_HASH_PREFIX}{digest}"


def verify_beacon_token(token: str, token_hash: str) -> bool:
    return secrets.compare_digest(hash_beacon_token(token), token_hash)
