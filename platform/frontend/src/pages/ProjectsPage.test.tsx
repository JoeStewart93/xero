...
describe('ProjectsPage', () => {
  ...
  it('auto-detects domains and IPs, rejects invalid IPs, and activates one project scope', () => {
    ...
    const initialScopeSelector = screen.getByLabelText('Active project scope') as HTMLButtonElement;
    expect(initialScopeSelector.textContent).toContain('Global');
    const initialActiveScope = screen.getByRole('complementary', { name: 'Project actions' });
    expect(within(initialActiveScope).getByText('Global')).toBeTruthy();
    ...
    expect(within(activeScope).getByText('Global')).toBeTruthy();
  });
  ...
  it('deletes a project only after confirmation and falls back to Global when deleting active scope', () => {
      ...
      JSON.stringify([
        {
          id: 'project-a',
          name: 'project a',
          targets: [{ id: 'target-a-domain', type: 'domain', value: 'a.example.com' }],
        },
        {
          id: 'project-b',
          name: 'project b',
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Delete project project a' }));
